...
TRIMMED=${PROJROOT}/02_trimmed_data/fastp_30
        ...
        for R1 in ${RAWFQ}/*R1.fq.gz ; #For each of read 1 in RAWFQ do the following.
          do
            ...
            fastp \
              ...
              -i ${R1} \
              -I ${R2} \
              -o ${TRIMMED}/trimmed_data/${R1BASE%_R1.fq.gz}_fastp30_R1.fq.gz \
              -O ${TRIMMED}/trimmed_data/${R2BASE%_R2.fq.gz}_fastp30_R2.fq.gz \
              --failed_out ${TRIMMED}/discarded/${R1BASE%_R1.fq.gz}_fastp30_discarded.fq.gz \
              --cut_right --cut_window_size 4 --cut_mean_quality 20 --length_required 30 \
              -j ${TRIMMED}/trimmed_data/${R1BASE%_R1.fq.gz}.json \
              -h ${TRIMMED}/trimmed_data/${R1BASE%_R1.fq.gz}.html \
              >> ${TRIMMED}/log/${R1BASE%_R1.fq.gz}_fastp30.log 2>&1
